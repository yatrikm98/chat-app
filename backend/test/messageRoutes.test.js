...
describe("Messages Routes", () => {
    describe('Creating Message ', () => {
        test('One of the data is not sent to back end', async () => {
            const res = await request(app).post('/api/message').send({ content: "", chatId: "" });
        ...
        test("Creating a message with credentials", async () => {
            ...
            const res = await request(app).post('/api/message').send({ content: "Hello", chatId: "123456" });
    ...
    describe('Fetching Messages', () => {
        test('Fetching Messages Successfull', async () => {
            ...
            const res = await request(app).get('/api/message/1234');
